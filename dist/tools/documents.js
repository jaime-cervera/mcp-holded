import { documentIdSchema, createDocumentSchema, updateDocumentSchema, updateDocumentPipelineSchema, attachFileToDocumentSchema, shipItemsByLineSchema, payDocumentSchema, sendDocumentSchema, updateDocumentTrackingSchema, withValidation, } from '../validation.js';
...
function applyTaxDefaults(items, docType) {
    ...
    return items.map((item) => {
        ...
        const taxes = [];
        if (DEFAULT_IVA_KEY)
            taxes.push(DEFAULT_IVA_KEY);
        if (DEFAULT_RETENTION_KEY)
            taxes.push(DEFAULT_RETENTION_KEY);
        if (taxes.length > 0) {
...
export function getDocumentTools(client) {
    return {
        // List Documents
        list_documents: {
            description: 'List all documents of a specific type with optional filters for date range, contact, payment status, and sorting. Supports field filtering to reduce response size.',
            inputSchema: {
                type: 'object',
                properties: {
                    docType: {
                    ...
                    fields: {
                        type: 'array',
                        items: { type: 'string' },
                        description: 'Select specific fields to return (e.g., ["id", "contactName", "total"]). Reduces response size by 70-90%. If not provided, returns default fields: id, contact, contactName, date, tax, total, status',
                    },
                },
                required: ['docType'],
            },
            ...
            handler: async (args) => {
                ...
                if (args.paid)
                    queryParams.paid = args.paid;
                if (args.billed)
                    queryParams.billed = args.billed;
                if (args.sort)
